import json
import time
import rclpy

from rclpy.action import ActionServer
from rclpy.node import Node

from std_msgs.msg import String
from smart_university_interfaces.action import CleanShelf


class CleaningActionServer(Node):

    def __init__(self):
        super().__init__('cleaning_action_server')

        self.declare_parameter('robot_id', 'robot_1')
        self.declare_parameter('cleaning_duration', 5.0)
        self.declare_parameter('feedback_period', 0.2)

        self.robot_id = self.get_parameter('robot_id').value
        self.cleaning_duration = float(
            self.get_parameter('cleaning_duration').value
        )
        self.feedback_period = float(
            self.get_parameter('feedback_period').value
        )

        self.world_events_pub = self.create_publisher(
            String,
            '/world_events',
            10
        )

        self.action_server = ActionServer(
            self,
            CleanShelf,
            f'/{self.robot_id}/clean_shelf',
            self.execute_cleaning
        )

        self.get_logger().info(
            f'Cleaning Action Server avviato per {self.robot_id}'
        )

    def execute_cleaning(self, goal_handle):
        task_id = goal_handle.request.task_id
        request_robot_id = goal_handle.request.robot_id
        target_id = goal_handle.request.target_id

        result = CleanShelf.Result()

        if request_robot_id != self.robot_id:
            goal_handle.abort()
            result.success = False
            result.reason = f'Robot errato: {request_robot_id}'
            return result

        if not task_id or not target_id:
            goal_handle.abort()
            result.success = False
            result.reason = 'task_id o target_id mancante'
            return result

        self.get_logger().info(
            f'Inizio pulizia: robot={self.robot_id}, '
            f'target={target_id}, task={task_id}'
        )

        start_time = time.time()

        while time.time() - start_time < self.cleaning_duration:
            if goal_handle.is_cancel_requested:
                goal_handle.canceled()
                result.success = False
                result.reason = 'Pulizia cancellata'
                return result

            elapsed = time.time() - start_time
            progress = min(1.0, elapsed / self.cleaning_duration)

            feedback = CleanShelf.Feedback()
            feedback.progress = float(progress)
            feedback.status = 'CLEANING'
            goal_handle.publish_feedback(feedback)

            time.sleep(self.feedback_period)

        self.publish_world_event(
            event_type='SHELF_CLEANED',
            target_id=target_id,
            task_id=task_id
        )

        goal_handle.succeed()

        result.success = True
        result.reason = ''

        self.get_logger().info(
            f'Pulizia completata: target={target_id}, task={task_id}'
        )

        return result

    def publish_world_event(self, event_type, target_id, task_id):
        event = {
            "event_type": event_type,
            "robot_id": self.robot_id,
            "target_id": target_id,
            "task_id": task_id,
            "timestamp": self.get_clock().now().nanoseconds
        }

        msg = String()
        msg.data = json.dumps(event)

        self.world_events_pub.publish(msg)


def main(args=None):
    rclpy.init(args=args)

    node = CleaningActionServer()

    try:
        rclpy.spin(node)
    except KeyboardInterrupt:
        pass

    node.destroy_node()
    rclpy.shutdown()


if __name__ == '__main__':
    main()